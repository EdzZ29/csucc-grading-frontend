<template>
  <div class="flex bg-gray-50 font-inria min-h-screen">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              ...
              >
                ...
                <div v-if="!card.hasData" class="px-5 py-6 flex flex-col items-center justify-center text-center gap-3">
                  ...
                  <span class="text-[10px] bg-gray-100 text-gray-500 px-3 py-1 rounded-full font-inria border border-gray-200">
                    Class imported ✓ · Grades pending
                  </span>
                </div>
                ...
                <div v-else class="px-5 py-4">
                    ...
                    <div class="relative flex-shrink-0">
                      <svg width="90" height="90" viewBox="0 0 90 90">
                        ...
                        <circle cx="45" cy="45" r="38" fill="none" stroke="#22c55e" stroke-width="14"
                          ...
                          :stroke-dashoffset="getPieOffset(card, 'safe')"
                          stroke-linecap="butt"
                          transform="rotate(-90 45 45)"
                          style="transition: stroke-dashoffset 0.6s ease;"/>
                        <circle cx="45" cy="45" r="38" fill="none" stroke="#f59e0b" stroke-width="14"
                          stroke-dasharray="238.76"
                          :stroke-dashoffset="getPieOffset(card, 'warning')"
                          stroke-linecap="butt"
                          :style="{ strokeDashoffset: getPieOffset(card, 'warning'), transform: `rotate(${getPieRotation(card, 'warning')}deg)`, transformOrigin: '45px 45px', transition: 'all 0.6s ease' }"/>
                        <circle cx="45" cy="45" r="38" fill="none" stroke="#ef4444" stroke-width="14"
                          stroke-dasharray="238.76"
                          :stroke-dashoffset="getPieOffset(card, 'critical')"
                          stroke-linecap="butt"
                          :style="{ strokeDashoffset: getPieOffset(card, 'critical'), transform: `rotate(${getPieRotation(card, 'critical')}deg)`, transformOrigin: '45px 45px', transition: 'all 0.6s ease' }"/>
                  ...
                  <div class="mt-3 grid grid-cols-2 gap-2">
                    <div class="bg-green-50 border border-green-200 rounded-lg px-3 py-2 text-center">
                      ...
                      <p class="text-lg font-epundaslab font-bold text-green-700">
                        {{ card.total > 0 ? Math.round(card.safe / card.total * 100) : 0 }}%
                      </p>
                    ...
                    <div class="bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-center">
                      ...
                      <p class="text-lg font-epundaslab font-bold text-red-700">
                        {{ card.total > 0 ? Math.round((card.warning + card.critical) / card.total * 100) : 0 }}%
                      </p>
    ...
    <!-- ── OBE Student Risk Modal (landscape: left panel + right student list) ── -->
      ...
      <div class="bg-white rounded-2xl w-full max-w-7xl flex flex-row overflow-hidden"
        ...
        <div class="w-72 flex-shrink-0 bg-gradient-to-b from-gray-800 to-gray-900 flex flex-col overflow-y-auto relative">
          ...
          <div class="px-5 pt-6 pb-4 border-b border-white/10 pr-12">
            <h2 class="text-lg font-epundaslab font-bold text-white leading-tight">
              {{ riskModal.card?.subjcode }}
            </h2>
          ...
          <div class="flex flex-col items-center py-5 border-b border-white/10">
            <svg width="120" height="120" viewBox="0 0 120 120">
              ...
              <circle cx="60" cy="60" r="50" fill="none" stroke="#22c55e" stroke-width="18"
                ...
                :stroke-dashoffset="getModalPieOffset('safe')"
                transform="rotate(-90 60 60)"
                style="transition: stroke-dashoffset 0.6s ease;"/>
              <!-- Warning -->
              <circle cx="60" cy="60" r="50" fill="none" stroke="#f59e0b" stroke-width="18"
                stroke-dasharray="314.16"
                :stroke-dashoffset="getModalPieOffset('warning')"
              ...
              <circle cx="60" cy="60" r="50" fill="none" stroke="#ef4444" stroke-width="18"
                stroke-dasharray="314.16"
                :stroke-dashoffset="getModalPieOffset('critical')"
            ...
            <!-- Pass / Fail big stat -->
            <div class="flex gap-3 mt-3 w-full px-4">
              <div class="flex-1 bg-green-500/20 border border-green-500/30 rounded-lg py-2 text-center">
                <p class="text-[10px] text-green-400 font-inria font-semibold uppercase">Passed</p>
                <p class="text-xl font-epundaslab font-bold text-green-300">
              ...
              <div class="flex-1 bg-red-500/20 border border-red-500/30 rounded-lg py-2 text-center">
                <p class="text-[10px] text-red-400 font-inria font-semibold uppercase">At Risk</p>
                <p class="text-xl font-epundaslab font-bold text-red-300">
                  {{ riskModal.students.length > 0 ? Math.round((riskModal.students.filter(s => s.risk_level !== 'Safe').length) / riskModal.students.length * 100) : 0 }}%
                </p>
              </div>
            </div>
          ...
          <div class="px-4 py-4 space-y-2 border-b border-white/10">
            <p class="text-[10px] text-gray-500 font-inria font-semibold uppercase tracking-wider mb-3">Risk Breakdown</p>

            <div class="flex items-center justify-between bg-green-500/10 border border-green-500/20 rounded-lg px-3 py-2">
              <div class="flex items-center gap-2">
                <span class="w-2 h-2 rounded-full bg-green-400"></span>
                <span class="text-xs text-green-300 font-inria">Safe</span>
              </div>
              <div class="text-right">
                <span class="text-sm font-bold text-green-300 font-epundaslab">{{ riskModal.students.filter(s => s.risk_level === 'Safe').length }}</span>
                <span class="text-[10px] text-green-500 font-inria ml-1">({{ riskModal.students.length > 0 ? Math.round(riskModal.students.filter(s=>s.risk_level==='Safe').length/riskModal.students.length*100) : 0 }}%)</span>
              </div>
            </div>
            ...
            <div class="flex items-center justify-between bg-yellow-500/10 border border-yellow-500/20 rounded-lg px-3 py-2">
              <div class="flex items-center gap-2">
                ...
                <span class="text-xs text-yellow-300 font-inria">Warning</span>
              </div>
              <div class="text-right">
                <span class="text-sm font-bold text-yellow-300 font-epundaslab">{{ riskModal.students.filter(s => s.risk_level === 'Warning').length }}</span>
                <span class="text-[10px] text-yellow-500 font-inria ml-1">({{ riskModal.students.length > 0 ? Math.round(riskModal.students.filter(s=>s.risk_level==='Warning').length/riskModal.students.length*100) : 0 }}%)</span>
              </div>
            </div>

            <div class="flex items-center justify-between bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">
              <div class="flex items-center gap-2">
                <span class="w-2 h-2 rounded-full bg-red-400"></span>
                <span class="text-xs text-red-300 font-inria">Critical</span>
              </div>
              <div class="text-right">
                <span class="text-sm font-bold text-red-300 font-epundaslab">{{ riskModal.students.filter(s => s.risk_level === 'Critical').length }}</span>
                <span class="text-[10px] text-red-500 font-inria ml-1">({{ riskModal.students.length > 0 ? Math.round(riskModal.students.filter(s=>s.risk_level==='Critical').length/riskModal.students.length*100) : 0 }}%)</span>
              </div>
            </div>

            <!-- Stacked bar -->
            <div class="mt-2 w-full h-2 bg-gray-700 rounded-full overflow-hidden flex">
              <div class="h-full bg-green-500 transition-all" :style="{ width: (riskModal.students.length > 0 ? riskModal.students.filter(s=>s.risk_level==='Safe').length/riskModal.students.length*100 : 0) + '%' }"></div>
              <div class="h-full bg-yellow-400 transition-all" :style="{ width: (riskModal.students.length > 0 ? riskModal.students.filter(s=>s.risk_level==='Warning').length/riskModal.students.length*100 : 0) + '%' }"></div>
              <div class="h-full bg-red-500 transition-all" :style="{ width: (riskModal.students.length > 0 ? riskModal.students.filter(s=>s.risk_level==='Critical').length/riskModal.students.length*100 : 0) + '%' }"></div>
            </div>
          </div>

          <!-- Most common weak COs across class -->
          <div class="px-4 py-4">
            <p class="text-[10px] text-gray-500 font-inria font-semibold uppercase tracking-wider mb-3">Top Weak COs in Class</p>
            <div v-if="topWeakCOs.length > 0" class="space-y-2">
              <div v-for="item in topWeakCOs" :key="item.co"
                class="flex items-center justify-between">
                <span class="text-xs text-gray-300 font-inria bg-red-500/20 border border-red-500/30 rounded px-2 py-0.5">{{ item.co }}</span>
                <div class="flex items-center gap-1">
                  <div class="w-16 h-1.5 bg-gray-700 rounded-full overflow-hidden">
                    <div class="h-full bg-red-400 rounded-full" :style="{ width: item.pct + '%' }"></div>
                  </div>
                  <span class="text-[10px] text-red-400 font-inria">{{ item.count }}</span>
                </div>
              </div>
            </div>
            <p v-else class="text-[10px] text-gray-600 font-inria italic">No weak COs detected.</p>
          </div>
        </div>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
          ...
          <div class="flex-shrink-0 border-b border-gray-200 bg-gray-50">
            ...
            <!-- Search + filter row (only shown on risk tab) -->
          ...
          <div class="flex-1 overflow-y-auto">
            ...
            <div v-if="riskModal.activeTab === 'risk'" class="px-5 py-4 space-y-3">
              ...
              <!-- Student card — unchanged from original -->
              <div v-for="(student, idx) in filteredModalStudents" :key="student.studid"
                ...
                <div class="flex items-stretch"
                  :class="{
                    'bg-red-50': student.risk_level === 'Critical',
                    'bg-yellow-50': student.risk_level === 'Warning',
                    'bg-green-50': student.risk_level === 'Safe'
                  }">
                  <div class="flex flex-col items-center justify-center px-4 py-3 gap-1 border-r border-white/60 min-w-[56px]"
                    :class="{
                      'bg-red-100': student.risk_level === 'Critical',
                      'bg-yellow-100': student.risk_level === 'Warning',
                      'bg-green-100': student.risk_level === 'Safe'
                    }">
                    <span class="text-[10px] font-bold text-gray-400">#{{ idx + 1 }}</span>
                    <div class="w-8 h-8 rounded-full flex items-center justify-center"
                      :class="{
                        'bg-red-500': student.risk_level === 'Critical',
                        'bg-yellow-500': student.risk_level === 'Warning',
                        'bg-green-500': student.risk_level === 'Safe'
                      }">
                  ...
                  <div class="flex items-center justify-center px-4 border-r border-white/60 flex-shrink-0">
                    <span v-if="student.risk_level === 'Critical'"
                      class="inline-flex items-center gap-1.5 bg-red-100 text-red-800 text-xs font-bold px-3 py-1.5 rounded-full border border-red-300">
                      <span class="w-2 h-2 rounded-full bg-red-500 flex-shrink-0"></span>Critical
                    </span>
                    <span v-else-if="student.risk_level === 'Warning'"
                      class="inline-flex items-center gap-1.5 bg-yellow-100 text-yellow-800 text-xs font-bold px-3 py-1.5 rounded-full border border-yellow-300">
                      <span class="w-2 h-2 rounded-full bg-yellow-500 flex-shrink-0"></span>Warning
                    </span>
                    <span v-else
                      class="inline-flex items-center gap-1.5 bg-green-100 text-green-800 text-xs font-bold px-3 py-1.5 rounded-full border border-green-300">
                  ...
                  <div class="flex flex-col justify-center px-4 py-3 w-40 flex-shrink-0 border-r border-white/60">
                    <div class="flex justify-between text-[10px] mb-1">
                      ...
                      <span class="font-bold"
                        :class="{
                          'text-red-600': student.fail_probability >= 70,
                          'text-yellow-600': student.fail_probability >= 40 && student.fail_probability < 70,
                          'text-green-600': student.fail_probability < 40
                        }">{{ student.fail_probability }}%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2">
                      <div class="h-2 rounded-full transition-all"
                        :class="{
                          'bg-red-500': student.fail_probability >= 70,
                          'bg-yellow-500': student.fail_probability >= 40 && student.fail_probability < 70,
                          'bg-green-500': student.fail_probability < 40
                        }"
                  ...
                  <div class="flex flex-col justify-center items-center px-4 py-3 w-28 flex-shrink-0 border-r border-white/60">
                    ...
                    <p class="font-bold text-base"
                      :class="student.co_pass_rate >= 0.75 ? 'text-green-700' : student.co_pass_rate >= 0.5 ? 'text-yellow-700' : 'text-red-700'">
                  ...
                  <div class="flex flex-col justify-center items-center px-4 py-3 w-28 flex-shrink-0 border-r border-white/60">
                    ...
                    <p class="font-bold text-base"
                      :class="{
                        'text-red-600': (student.total_weighted_percent || 0) < 60,
                        'text-yellow-600': (student.total_weighted_percent || 0) >= 60 && (student.total_weighted_percent || 0) < 75,
                        'text-green-600': (student.total_weighted_percent || 0) >= 75
                      }">
                  ...
                  <div class="flex flex-wrap items-center gap-1 px-4 py-3 flex-1 min-w-0">
                    <template v-if="student.weak_cos && student.weak_cos.length > 0">
                      ...
                      <span v-for="co in student.weak_cos" :key="co"
                        class="bg-red-100 text-red-700 text-[10px] font-bold px-2 py-0.5 rounded-full border border-red-200 flex-shrink-0">
                        {{ co }}
                      </span>
                    </template>
                    <span v-else class="text-[10px] text-gray-400 font-inria italic">
                      {{ student.risk_level === 'Safe' ? 'All COs on track' : 'No CO data' }}
                    </span>
                  </div>
                </div>
                <!-- Bottom row -->
                <div class="grid grid-cols-2 border-t"
                  :class="{
                    'border-red-200 bg-white': student.risk_level === 'Critical',
                    'border-yellow-200 bg-white': student.risk_level === 'Warning',
                    'border-green-200 bg-white': student.risk_level === 'Safe'
                  }">
            ...
            <div v-if="riskModal.activeTab === 'heatmap'" class="p-5">
              ...
              <div v-else-if="riskModal.heatmapData && riskModal.heatmapData.error"
                ...
                <div class="bg-gray-50 rounded-xl border border-gray-200 p-4">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <div v-for="co in riskModal.heatmapData.co_summary" :key="co.co"
                      ...
                      <p class="text-[10px] font-bold font-inria"
                        :class="co.pass_rate >= 75 ? 'text-green-700' : co.pass_rate >= 50 ? 'text-yellow-700' : 'text-red-700'">
                        {{ co.co }}
                      </p>
                      <p class="text-lg font-epundaslab font-bold"
                        :class="co.pass_rate >= 75 ? 'text-green-800' : co.pass_rate >= 50 ? 'text-yellow-800' : 'text-red-800'">
                        {{ co.pass_rate }}%
                      </p>
                      ...
                      <div class="w-full bg-gray-200 rounded-full h-1 mt-1.5">
                        <div class="h-1 rounded-full"
                          :class="co.pass_rate >= 75 ? 'bg-green-500' : co.pass_rate >= 50 ? 'bg-yellow-500' : 'bg-red-500'"
                          :style="{ width: co.pass_rate + '%' }"></div>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <table class="w-full text-sm">
                    <thead>
                      <tr class="border-b border-gray-200 bg-gray-50">
                        ...
                        <th v-for="co in riskModal.heatmapData.cos" :key="co"
                          class="px-3 py-2.5 text-center text-xs font-bold uppercase tracking-wider min-w-[72px]">{{ co }}</th>
                        <th class="px-4 py-2.5 text-center text-xs text-gray-500 font-semibold uppercase">Avg</th>
                      </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                      <tr v-for="(student, idx) in filteredHeatmapStudents" :key="student.studid"
                        ...
                        <td v-for="score in student.scores" :key="score.co" class="px-2 py-2 text-center">
                          <div class="w-14 h-7 rounded-lg flex items-center justify-center text-xs font-bold mx-auto"
                            :class="{
                              'bg-green-100 text-green-800 border border-green-200': score.status === 'pass',
                              'bg-red-100 text-red-800 border border-red-200': score.status === 'fail',
                              'bg-gray-100 text-gray-400 border border-gray-200': score.status === 'missing'
                            }">
                        ...
                        <td class="px-4 py-2 text-center">
                          <span class="font-bold text-sm"
                            :class="student.avg_score >= 75 ? 'text-green-700' : student.avg_score >= 60 ? 'text-yellow-700' : 'text-red-700'">
                            {{ student.avg_score }}%
                          </span>
                        </td>
                      </tr>
            ...
            <div v-if="riskModal.activeTab === 'trajectory'" class="p-5 space-y-4">
              ...
              <div v-else-if="riskModal.trajectoryData && riskModal.trajectoryData.error"
                class="flex flex-col items-center justify-center py-16 text-gray-400">
                <svg class="w-14 h-14 text-gray-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
              ...
              <div v-else-if="riskModal.trajectoryData">
                <div v-for="student in filteredTrajectoryStudents" :key="student.studid"
                  ...
                  :class="{
                    'border-red-200': student.trend === 'declining',
                    'border-gray-200': student.trend === 'stable',
                    'border-green-200': student.trend === 'improving'
                  }">

                  <!-- Student header -->
                  <div class="flex items-center justify-between px-4 py-3 border-b"
                    :class="{
                      'bg-red-50 border-red-100': student.trend === 'declining',
                      'bg-gray-50 border-gray-100': student.trend === 'stable',
                      'bg-green-50 border-green-100': student.trend === 'improving'
                    }">
                    <div class="flex items-center gap-3">
                      <div class="w-7 h-7 rounded-full flex items-center justify-center"
                        :class="{
                          'bg-red-500': student.trend === 'declining',
                          'bg-gray-400': student.trend === 'stable',
                          'bg-green-500': student.trend === 'improving'
                        }">
                    ...
                    <div class="flex items-center gap-4">
                      <div class="text-center">
                        ...
                        <p class="font-bold text-sm"
                          :class="student.overall_avg >= 75 ? 'text-green-700' : student.overall_avg >= 60 ? 'text-yellow-700' : 'text-red-700'">
                          {{ student.overall_avg }}%
                        </p>
                      </div>
                      <div class="text-center">
                        ...
                        <p class="font-bold text-sm"
                          :class="student.latest_avg >= 75 ? 'text-green-700' : student.latest_avg >= 60 ? 'text-yellow-700' : 'text-red-700'">
                          {{ student.latest_avg }}%
                        </p>
                      </div>
                      <span class="text-xs font-bold px-3 py-1 rounded-full border font-inria"
                        :class="{
                          'bg-red-100 text-red-700 border-red-200': student.trend === 'declining',
                          'bg-gray-100 text-gray-600 border-gray-200': student.trend === 'stable',
                          'bg-green-100 text-green-700 border-green-200': student.trend === 'improving'
                        }">
                  ...
                  <div class="px-4 py-3 overflow-x-auto">
                    <div class="flex items-end gap-1 min-w-max" style="height: 52px;">
                      <div v-for="(pt, i) in student.points" :key="i"
                        class="group/bar flex flex-col items-center gap-0.5 cursor-default"
                        style="min-width: 28px;">
                        <div class="relative w-6 rounded-t transition-all"
                          :style="{ height: pt.pct !== null ? Math.max((pt.pct / 100) * 40, 3) + 'px' : '3px' }"
                          :class="{
                            'bg-green-400': pt.status === 'pass',
                            'bg-red-400': pt.status === 'fail',
                            'bg-gray-200': pt.status === 'missing'
                          }">
                          <!-- Tooltip -->
                    ...
                    <div class="flex gap-1 mt-1 min-w-max">
                      <div v-for="(pt, i) in student.points" :key="'l'+i"
                        style="min-width: 28px;"
                        class="text-[9px] text-gray-400 font-inria text-center truncate">
                  ...
                  <!-- Declining alert -->
                  <div v-if="student.trend === 'declining'"
                    class="px-4 py-2 bg-red-50 border-t border-red-100 flex items-center gap-2">
...
export default {
  ...
  data() {
    return {
      ...
      // ── OBE Dashboard ──
      obeLoading: false,
      obeClassCards: [],          // [{ key, subjcode, section, sy, sem, instructor, total, safe, warning, critical, students[] }]
      riskModal: {
        open: false,
        loading: false,
        card: null,
    ...
    filteredModalStudents() {
      ...
      if (this.riskModal.search.trim()) {
        ...
        list = list.filter(s =>
          ...
          String(s.studid || '').toLowerCase().includes(q)
        )
      }
      return list
    },
    // Aggregate weak COs across all students in the open modal, sorted by frequency
    topWeakCOs() {
      const counts = {}
      this.riskModal.students.forEach(s => {
        if (s.weak_cos && s.weak_cos.length) {
          s.weak_cos.forEach(co => { counts[co] = (counts[co] || 0) + 1 })
        }
      })
      ...
      return Object.entries(counts)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 6)
        .map(([co, count]) => ({ co, count, pct: Math.round(count / total * 100) }))
    },
  ...
  methods: {
    ...
    setActivePage(item) {
      ...
      if (item.name === 'Dashboard') {
        this.fetchDashboardStats()
        if (this.obeClassCards.length === 0) {
          this.fetchAllClassRisks()
        }
      }
    },
    ...
    // ── OBE Dashboard methods ────────────────────────────────────────

    // Step 1: Always build cards from masterlist first, then try to attach prediction data
    async fetchAllClassRisks() {
      ...
      try {
        ...
        // Group into unique class keys — every class shows regardless of prediction data
        const classMap = {}
        lists.forEach(row => {
          ...
          if (!classMap[key]) {
            classMap[key] = {
              ...
              instructor: row.employee
                ...
                : 'Unassigned',
              // defaults — shown while prediction loads or if no data yet
              hasData: false,
              total: 0,
              safe: 0,
              warning: 0,
        ...
        // Show cards immediately from masterlist — no waiting for predictions
        const classes = Object.values(classMap)
        this.obeClassCards = classes
        this.obeLoading = false

        // Now fetch predictions in parallel and update each card reactively
        const results = await Promise.allSettled(
          classes.map(cls =>
            this.$axios.get('/prediction/batch', {
        ...
        results.forEach(result => {
          ...
          // Reactively update the card in place
          this.$set(this.obeClassCards, cardIndex, {
            ...this.obeClassCards[cardIndex],
            hasData: true,
            total: data.length,
            safe,
            warning,
        ...
        // Re-sort after predictions arrive: cards with data (most critical) first, no-data cards last
        this.obeClassCards = [
        ...
        console.error("Failed to load OBE class risks", e)
        this.obeLoading = false
      }
    },

    // Step 2: Open modal with the already-fetched students
    openRiskModal(card) {
      this.riskModal.card = card
      this.riskModal.students = [...card.students].sort((a, b) => b.fail_probability - a.fail_probability)
      this.riskModal.filter = ''
      this.riskModal.search = ''
      this.riskModal.activeTab = 'risk'
      this.riskModal.heatmapData = null
      this.riskModal.heatmapLoading = false
      this.riskModal.heatmapSearch = ''
      this.riskModal.trajectoryData = null
      this.riskModal.trajectoryLoading = false
      this.riskModal.trajectorySearch = ''
      this.riskModal.trajectoryFilter = ''
      this.riskModal.open = true
    },

    closeRiskModal() {
      this.riskModal.open = false
      this.riskModal.card = null
      this.riskModal.students = []
      this.riskModal.filter = ''
      this.riskModal.search = ''
      this.riskModal.activeTab = 'risk'
      this.riskModal.heatmapData = null
    ...
    // ── Switch modal tab — lazy-load heatmap/trajectory on first visit ──
    switchModalTab(tab) {
      this.riskModal.activeTab = tab
      if (tab === 'heatmap' && !this.riskModal.heatmapData && !this.riskModal.heatmapLoading) {
        this.loadModalHeatmap()
      }
      if (tab === 'trajectory' && !this.riskModal.trajectoryData && !this.riskModal.trajectoryLoading) {
        this.loadModalTrajectory()
      }
    },

    async loadModalHeatmap() {
      const card = this.riskModal.card
      if (!card) return
      this.riskModal.heatmapLoading = true
      try {
        const res = await this.$axios.get('/prediction/heatmap', {
          params: { subjcode: card.subjcode, section: card.section, sy: card.sy, sem: card.sem },
          withCredentials: true
        })
        if (res.data && !res.data.error && res.data.students && res.data.students.length) {
          this.riskModal.heatmapData = res.data
        } else {
          this.riskModal.heatmapData = { error: res.data?.error || 'No CO score data available.' }
        }
      } catch (e) {
        this.riskModal.heatmapData = { error: 'Failed to load heatmap data.' }
      } finally {
        this.riskModal.heatmapLoading = false
      }
    },
    ...
    async loadModalTrajectory() {
      ...
      try {
        const res = await this.$axios.get('/prediction/trajectory', {
          params: { subjcode: card.subjcode, section: card.section, sy: card.sy, sem: card.sem },
          withCredentials: true
        })
        if (res.data && !res.data.error && res.data.students && res.data.students.length) {
          this.riskModal.trajectoryData = res.data
        } else {
          this.riskModal.trajectoryData = { error: res.data?.error || 'No assessment data available.' }
        }
      } catch (e) {
    ...
    // ── Pie chart helpers ────────────────────────────────────────────
    // SVG donut: circumference = 2 * π * 38 ≈ 238.76
    getPieOffset(card, segment) {
      const circ = 238.76
      if (card.total === 0) return circ
      const pct = segment === 'safe'
        ? card.safe / card.total
        : segment === 'warning'
          ? card.warning / card.total
          : card.critical / card.total
      return circ * (1 - pct)
    },

    getPieRotation(card, segment) {
      if (card.total === 0) return -90
      const safePct     = card.safe / card.total
      const warningPct  = card.warning / card.total
      if (segment === 'warning') {
        return -90 + safePct * 360
      }
      // critical
      return -90 + (safePct + warningPct) * 360
    },

    // ── Risk explanation generator ───────────────────────────────────
    getRiskExplanation(student) {
      const fp   = student.fail_probability || 0
      const copr = student.co_pass_rate !== undefined ? student.co_pass_rate * 100 : null
      const wp   = student.total_weighted_percent || 0
      const weak = student.weak_cos || []
      ...
      if (student.risk_level === 'Critical') {
        ...
        return reasons.length
          ? `This student is at critical risk due to: ${reasons.join('; ')}. Immediate academic intervention is strongly recommended.`
          : 'This student has indicators placing them at critical academic risk. Immediate intervention is recommended.'
      }
      ...
      if (student.risk_level === 'Warning') {
        ...
        return reasons.length
          ? `This student shows warning signs: ${reasons.join('; ')}. Close monitoring and early support are advised.`
          : 'This student shows early warning indicators. Proactive support is advised to prevent further decline.'
      }
      ...
      return positives.length
        ? `This student is performing well with ${positives.join(', ')}. No immediate intervention required.`
        : 'This student is on track. Performance indicators are within acceptable range.'
    },

    // ── Modal donut pie helpers (r=50, circ=314.16) ─────────────────
    getModalPieOffset(segment) {
      const circ = 314.16
      const students = this.riskModal.students
      const total = students.length
      if (total === 0) return circ
      const count = segment === 'safe'
        ? students.filter(s => s.risk_level === 'Safe').length
        : segment === 'warning'
          ? students.filter(s => s.risk_level === 'Warning').length
    ...
    getModalPieRotation(segment) {
      ...
      if (segment === 'warning') return -90 + safePct * 360
      return -90 + (safePct + warningPct) * 360
    },

    // ── Per-CO assessment intervention — short & direct ──────────────
    getAssessmentIntervention(co, student) {
      const fp = student.fail_probability || 0
      const wp = student.total_weighted_percent || 0
      if (fp >= 70 || wp < 60) {
        return 'Remediation & re-assessment needed'
      }
      return 'Review & practice recommended'
    }
  }
}
</script>
