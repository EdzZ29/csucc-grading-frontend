            ...
            <div class="w-full xl:w-96 bg-white rounded-2xl shadow-md p-6">
              ...
              <div class="grid grid-cols-7 text-center gap-1">
                <span v-for="n in 31" :key="n" :class="[
                  ...
                  n === currentDay ? 'bg-orange400 text-white shadow-md font-bold' : 'text-gray-600 hover:bg-orange100 hover:text-orange400'
                ]">{{ n }}</span>
              </div>
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              <div v-for="card in paginatedObeClassCards" :key="card.key"
                ...
                <div v-if="!card.hasData" class="px-5 py-6 flex flex-col items-center justify-center text-center gap-3">
                  ...
                  <div>
                    <p class="text-sm font-bold text-gray-500 font-inria">Awaiting Score Data</p>
                    <p class="text-[10px] text-gray-400 font-inria mt-1">Enter at least one assessment<br>score in the Grading Module.</p>
                  </div>
                  <span class="text-[10px] bg-gray-100 text-gray-500 px-3 py-1 rounded-full font-inria border border-gray-200">
                    Class imported ✓ · Scores pending
                  </span>
                </div>

                <!-- Has prediction data (full OR partial/early) -->
                <div v-else class="px-5 py-4">

                  <!-- Early prediction banner -->
                  <div v-if="card.isPartial"
                    class="mb-3 flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-lg px-3 py-1.5">
                    <span class="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse flex-shrink-0"></span>
                    <p class="text-[10px] font-bold text-blue-700 font-inria">
                      Early Prediction · {{ Math.round((card.completionRate || 0) * 100) }}% filled
                    </p>
                    <span class="ml-auto text-[9px] text-blue-500 font-inria bg-blue-100 px-1.5 py-0.5 rounded-full whitespace-nowrap">
                      {{ card.cosWithData }}/{{ card.totalCos || '?' }} COs
                    </span>
                  </div>
                    ...
                    <div class="relative flex-shrink-0">
                      <svg width="90" height="90" viewBox="0 0 90 90">
                        ...
                        <circle cx="45" cy="45" r="38" fill="none" stroke="#22c55e" stroke-width="14"
                          stroke-dasharray="238.76" :stroke-dashoffset="getPieOffset(card, 'safe')"
                          stroke-linecap="butt" transform="rotate(-90 45 45)" style="transition:stroke-dashoffset 0.6s ease;"/>
                        <circle cx="45" cy="45" r="38" fill="none" stroke="#f59e0b" stroke-width="14"
                          stroke-dasharray="238.76" :stroke-dashoffset="getPieOffset(card, 'warning')"
                          stroke-linecap="butt"
                        ...
                        <circle cx="45" cy="45" r="38" fill="none" stroke="#ef4444" stroke-width="14"
                          stroke-dasharray="238.76" :stroke-dashoffset="getPieOffset(card, 'critical')"
                          stroke-linecap="butt"
                  ...
                  <div class="mt-3 grid grid-cols-2 gap-2">
                    <div class="bg-green-50 border border-green-200 rounded-lg px-3 py-2 text-center">
                      <p class="text-[10px] text-green-600 font-inria font-semibold uppercase tracking-wide">
                        {{ card.isPartial ? 'On Track' : 'Class Passed' }}
                      </p>
                  ...
                  <!-- Completion bar (partial only) -->
                  <div v-if="card.isPartial" class="mt-3">
                    <div class="flex justify-between text-[10px] text-gray-400 font-inria mb-1">
                      <span>Gradebook completion</span>
                      <span>{{ Math.round((card.completionRate || 0) * 100) }}%</span>
                    </div>
                    <div class="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
                      <div class="h-full bg-blue-400 rounded-full transition-all"
                        :style="{ width: ((card.completionRate || 0) * 100) + '%' }"></div>
                    </div>
                    <p class="text-[9px] text-gray-400 font-inria mt-1">
                      Confidence improves as more assessments are entered
                    </p>
                  </div>

                  <!-- Footer -->
                  <div class="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between">
                    <span class="text-[10px] text-gray-400 font-inria">
                      {{ card.total }} students · {{ card.isPartial ? '⚡ Live prediction' : '✓ Final grades' }}
                    </span>
      ...
      <div class="bg-white rounded-2xl w-full max-w-7xl flex flex-row overflow-hidden"
        ...
        <div class="w-72 flex-shrink-0 bg-gradient-to-b from-gray-800 to-gray-900 flex flex-col overflow-y-auto relative">
          ...
          <div class="px-5 pt-6 pb-4 border-b border-white/10 pr-12">
            ...
            <!-- Partial indicator in modal -->
            <div v-if="riskModal.card?.isPartial" class="mt-3 flex items-center gap-1.5 bg-blue-500/10 border border-blue-500/20 rounded-lg px-3 py-1.5">
              <span class="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse"></span>
              <p class="text-[10px] text-blue-300 font-inria font-bold">Live · Early Prediction</p>
            </div>
          </div>
            ...
            <div class="bg-green-500/10 border border-green-500/20 rounded-xl px-4 py-3">
              ...
              <div class="flex items-end justify-between">
                ...
                <p class="text-xs text-green-400 font-inria mb-1">
                  {{ riskModal.students.filter(s => s.risk_level === 'Safe').length }} / {{ riskModal.students.length }}
                </p>
              </div>
            ...
            <div v-if="riskModal.activeTab === 'trajectory'" class="flex items-center gap-3 px-6 py-3">
              ...
              <div v-if="riskModal.trajectoryData && riskModal.trajectoryData.students" class="flex gap-2">
                <span v-for="t in ['declining','stable','improving']" :key="t"
                  ...
                  :class="{ 'bg-red-50 border-red-200 text-red-700': t==='declining', 'bg-gray-50 border-gray-200 text-gray-600': t==='stable', 'bg-green-50 border-green-200 text-green-700': t==='improving' }">
          ...
          <div class="flex-1 overflow-y-auto">
            ...
            <div v-if="riskModal.activeTab === 'heatmap'" class="p-5">
              ...
              <div v-else-if="riskModal.heatmapData && riskModal.heatmapData.error"
                ...
                <p class="text-sm font-inria">Enter scores in the Grading Module first.</p>
              </div>
              ...
              <div v-else-if="riskModal.trajectoryData">
                <div v-for="student in filteredTrajectoryStudents" :key="student.studid"
                  ...
                  <div v-if="student.trend === 'declining'" class="px-4 py-2 bg-red-50 border-t border-red-100 flex items-center gap-2">
                    ...
                    <p class="text-[10px] text-red-700 font-inria">Declining trend — early intervention recommended.</p>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    setActivePage(item) {
      this.activePage = item; this.sidebarOpen = false
      if (item.name === 'Dashboard') { this.fetchDashboardStats(); if (!this.obeClassCards.length) this.fetchAllClassRisks() }
    },
    ...
    async confirmLogout() {
      try { await this.$axios.post("/auth/logout", {}, { withCredentials: true }); this.$store.dispatch('logout'); this.user = null; window.location.href = "/" }
      catch (err) { console.error("Logout failed:", err) }
    },
    handleScroll() { this.showScrollTop = window.scrollY > 200 },
    scrollToTop() { window.scrollTo({ top: 0, behavior: "smooth" }) },

    // ── UPDATED: supports early/partial predictions ──────────────────
    async fetchAllClassRisks() {
      ...
      try {
        ...
        lists.forEach(row => {
          ...
          if (!classMap[key]) {
            classMap[key] = {
              ...
              hasData: false,
              isPartial: false,       // true = prediction from partial semester data
              completionRate: 0,      // 0-1 fraction of gradebook filled
              cosWithData: 0,         // COs that have score entries
              totalCos: 0,            // total COs in syllabus
              total: 0, safe: 0, warning: 0, critical: 0, students: []
            }
          }
        })
        ...
        // Fetch predictions in parallel — works with partial OR complete data
        ...
        results.forEach(result => {
          ...
          if (!data || data.error || !Array.isArray(data) || !data.length) return
          ...
          // Extract partial metadata from batch response
          const isPartial      = data.some(s => s.is_partial === true)
          const completionRate = data.reduce((sum, s) => sum + (s.completion_rate || 0), 0) / data.length
          const first          = data[0] || {}
          const cosWithData    = first.cos_with_data ?? (first.co_coverage ? Math.round((first.co_coverage || 0) * (first.total_cos || 4)) : 0)
          const totalCos       = first.total_cos ?? 0
          ...
          this.$set(this.obeClassCards, cardIndex, {
            ...
            cosWithData,
            totalCos,
            total: data.length, safe, warning, critical, students: data
          })
        })

        // Sort: critical first, then warning, then no-data
        this.obeClassCards = [
          ...this.obeClassCards.filter(c => c.hasData && c.critical > 0).sort((a, b) => b.critical - a.critical),
          ...this.obeClassCards.filter(c => c.hasData && c.critical === 0).sort((a, b) => b.warning - a.warning),
    ...
    async loadModalHeatmap() {
      ...
      try {
        ...
        this.riskModal.heatmapData = (res.data && !res.data.error && res.data.students?.length) ? res.data : { error: res.data?.error || 'No CO score data available.' }
    ...
    async loadModalTrajectory() {
      ...
      try {
        ...
        this.riskModal.trajectoryData = (res.data && !res.data.error && res.data.students?.length) ? res.data : { error: res.data?.error || 'No assessment data available.' }
      } catch (e) { this.riskModal.trajectoryData = { error: 'Failed to load trajectory data.' } }
      finally { this.riskModal.trajectoryLoading = false }
    },

    // Pie helpers (card, r=38, circ=238.76)
    ...
    getPieRotation(card, segment) {
      ...
      const safePct = card.safe/card.total, warnPct = card.warning/card.total
      return segment==='warning' ? -90 + safePct*360 : -90 + (safePct+warnPct)*360
    },
    // Modal pie (r=50, circ=314.16)
    getModalPieOffset(segment) {
      const circ = 314.16, students = this.riskModal.students, total = students.length
    ...
    getModalPieRotation(segment) {
      const students = this.riskModal.students, total = students.length
      if (!total) return -90
      const safePct = students.filter(s=>s.risk_level==='Safe').length/total
      const warnPct = students.filter(s=>s.risk_level==='Warning').length/total
      return segment==='warning' ? -90 + safePct*360 : -90 + (safePct+warnPct)*360
    },
  }
}
