<template>
  <div class="flex bg-gray-50 font-inria min-h-screen">
        ...
        <div v-if="activePage.name === 'Dashboard'" class="animate-fade-in space-y-8">
            ...
            <div class="flex-1 bg-gradient-to-br from-orange400 to-orange300 rounded-2xl p-8 text-white shadow-lg relative overflow-hidden flex flex-col justify-center min-h-[250px]">
              ...
              <div class="relative z-10">
                <div class="flex flex-wrap items-center gap-2 mb-4">
                  <span class="bg-white/20 px-3 py-1 rounded-full text-xs font-inria">CHAIRPERSON DASHBOARD</span>
                ...
                <h1 class="text-3xl md:text-4xl font-epundaslab font-bold mb-3">
                  Welcome, Chairperson {{ user?.lastname || 'Chairperson' }}! 🎓
                </h1>
                <p class="text-white/90 text-lg font-inria leading-relaxed max-w-xl">
                  Monitor academic performance across all classes and instructors. View OBE risk levels, track student progress, and oversee intervention recommendations institution-wide.
                </p>

                <div class="flex flex-wrap gap-4 mt-6">
                  <div class="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-full">
                    <div class="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
                    <span class="text-sm font-inria">{{ masterlistStats.count }} Total Classes</span>
                  </div>
          ...
          <div>
            ...
            <div v-else-if="!obeLoading && obeClassCards.length === 0"
              ...
              <p class="font-medium font-epundaslab text-lg mb-1">No classes assigned yet</p>
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              ...
              >
                ...
                <div v-else class="px-5 py-4">
                  ...
                  <!-- Pass / Fail percentage summary — real-time from actual scores -->
                  <div class="mt-3 grid grid-cols-2 gap-2">
                    <div class="bg-green-50 border border-green-200 rounded-lg px-3 py-2 text-center">
                      <p class="text-[10px] text-green-600 font-inria font-semibold uppercase tracking-wide flex items-center justify-center gap-1">
                        Class Passed
                        <span v-if="card.anyPartial" class="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" title="Based on scores entered so far"></span>
                      </p>
                      <p class="text-lg font-epundaslab font-bold text-green-700">
                        {{ card.total > 0 ? Math.round((card.realPassed || 0) / card.total * 100) : 0 }}%
                      </p>
                      <p class="text-[10px] text-green-500 font-inria">{{ card.realPassed || 0 }} / {{ card.total }} students</p>
                    </div>
                    <div class="bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-center">
                      <p class="text-[10px] text-red-600 font-inria font-semibold uppercase tracking-wide flex items-center justify-center gap-1">
                        At Risk
                        <span v-if="card.anyPartial" class="w-1.5 h-1.5 bg-red-400 rounded-full animate-pulse" title="Based on scores entered so far"></span>
                      </p>
                      <p class="text-lg font-epundaslab font-bold text-red-700">
                        {{ card.total > 0 ? Math.round((card.realAtRisk || 0) / card.total * 100) : 0 }}%
                      </p>
                      <p class="text-[10px] text-red-500 font-inria">{{ card.realAtRisk || 0 }} / {{ card.total }} students</p>
                    </div>
                  </div>
    ...
    <!-- ═══════════════════════════════════════════════ -->
    <!-- OBE STUDENT RISK MODAL (landscape)              -->
    <!-- ═══════════════════════════════════════════════ -->
...
export default {
  ...
  methods: {
    ...
    async fetchDashboardStats() {
      try {
        const res = await this.$axios.get("/masterlist/all-classes", { withCredentials: true })
    ...
    async fetchAllClassRisks() {
      ...
      try {
          ...
          if (!classMap[key]) {
            classMap[key] = {
              ...
              instructor: row.employee
                ...
                : (this.user ? `${this.user.firstname} ${this.user.lastname}` : 'Instructor'),
              // defaults shown while predictions load or if no grade data yet
              hasData: false,
              total: 0,
              safe: 0,
              warning: 0,
        ...
        // Show cards immediately — don't wait for predictions
        const classes = Object.values(classMap)
        this.obeClassCards = classes
        this.obeLoading = false

        // Fetch predictions in parallel, update each card reactively when ready
        ...
        results.forEach(result => {
          ...
          // ── Real-time pass / at-risk counts ─────────────────────────
          // Based on co_pass_rate (fraction of entered COs that passed ≥60%)
          // Works mid-semester: if only CO1 is entered and student passed it,
          // they count as passing. No final grade required.
          const PASS_THRESHOLD = 60
          const realPassed = data.filter(s => {
            if (s.co_pass_rate !== undefined && s.co_pass_rate !== null) {
              return (s.co_pass_rate * 100) >= PASS_THRESHOLD
            }
            return (s.total_weighted_percent || 0) >= PASS_THRESHOLD
          }).length
          const realAtRisk = data.length - realPassed
          const anyPartial = data.some(s => s.is_partial)
        ...
        // Re-sort: cards with data (most critical) first, no-data cards last
        this.obeClassCards = [
    ...
    getAssessmentIntervention(co, student) {
      ...
      const wp = student.total_weighted_percent || 0
      if (fp >= 70 || wp < 60) {
        return `Immediate remediation required for ${co}. Recommend one-on-one tutoring, targeted re-teaching of core concepts, and additional formative assessments for this outcome. Student is at high risk of failing this component.`
      }
      return `Student needs focused support on ${co}. Supplementary exercises, peer study groups, or guided practice sessions are advised to strengthen performance on this outcome before final evaluation.`
    }
  }
}
</script>
