            ...
            <div
              ...
              <div class="relative z-10 max-w-lg">
                <h1 class="text-3xl md:text-4xl font-bold font-epundaslab mb-3 leading-tight">
                  Welcome, Chancellor! 🏛️
                </h1>
                <p class="text-green-100 text-lg leading-relaxed">
                  Overview of the university's academic performance and enrollment statistics.
                </p>
              </div>
              <div
          ...
          <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div
                ...
                <div>
                  <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Total Enrolled Students</p>
                  <h4 class="text-4xl font-bold text-green-900 group-hover:text-green-700 transition-colors">{{
                    dashboardStats.students }}</h4>
                  <p class="text-sm text-gray-500 mt-2">Unique students across all masterlists</p>
                </div>
                <div
                  class="h-16 w-16 bg-blue-50 rounded-full flex items-center justify-center group-hover:bg-blue-100 transition-colors">
                  <img src="../../assets/image/multiple-users-silhouette.png" class="w-8 h-8 opacity-80" alt="icon">
                </div>
              </div>
              ...
              <div
                class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between hover:shadow-md transition-shadow group">
                <div>
                  <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Total Masterlists</p>
                  <h4 class="text-4xl font-bold text-green-900 group-hover:text-green-700 transition-colors">{{
                    dashboardStats.masterlists }}</h4>
                  <p class="text-sm text-gray-500 mt-2">Total active class records</p>
                </div>
                <div
...
export default {
  ...
  methods: {
    async fetchStats() {
      try {
        // Fetch all masterlists to calculate stats
        const res = await axios.get("http://localhost:9000/api/masterlist/all", { withCredentials: true });
        const lists = res.data;

        // 1. Total Masterlists (Unique Classes)
        const uniqueClasses = new Set();
        lists.forEach(item => {
          if (item.subjcode && item.section) uniqueClasses.add(`${item.subjcode}-${item.section}`);
        });
        this.dashboardStats.masterlists = uniqueClasses.size;

        // 2. Total Students (Unique Student IDs)
        const uniqueStudents = new Set();
        lists.forEach(item => {
          if (item.studid) uniqueStudents.add(item.studid);
        });
        this.dashboardStats.students = uniqueStudents.size;
